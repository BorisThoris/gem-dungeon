...
function candidate(seed: string | number, floors = 1): DungeonValidationInput {
...
describe("non-mutating canonical validation", () => {
  ...
  test("validates explicit vertical traversal across three floors", () => {
    const report = validateDungeon(candidate("validation-floors", 3));
  ...
  test("reports a broken corridor without repairing or mutating it", () => {
    const dungeon = candidate("validation-corruption");
